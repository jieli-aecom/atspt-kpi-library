...
`CURRENT_SCHEMA_VERSION` in [`src/types.ts`](src/types.ts) is the authoritative schema version. The current version is `44`.
...
Scenario tags, scenario-dependent sources, and schema 44 migration behavior are documented in [docs/SCENARIOS.md](docs/SCENARIOS.md).
