...
`CURRENT_SCHEMA_VERSION` in [`src/types.ts`](src/types.ts) is the authoritative schema version. The current version is `28`.
...
- `lookups`: reusable lookup definitions with documented inputs, output, and optional long-form paragraphed text
