...
import { fallbackRelationKeyName, synchronizeRelationKeyNames, updateRelationFieldRole } from './tableRelationNames';
...
import {
  ...
  reconcileRelationFields,
  setPrincipalFieldExpanded
} from './configSchema';
...
function DataSourceHeader({
  ...
  const [relationEditor, setRelationEditor] = useState<{
    ...
    anchor: 'primaryKey' | 'table' | 'linkedField' | 'tableLink';
    relationId?: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (editRequest.kind === 'dataField') {
      ...
      setCollapsedSourceCategories((current) => current.filter((category) => category !== (sourceGroup?.category ?? source?.category ?? 'Preprocessed Constants')));
      if (sourceGroup) {
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      const target = event.target;
      if (target instanceof Element && target.closest('.data-source-field-move-menu, .field-relation-popover')) return;
  ...
  const addLibraryGroup = (kind: LibraryKind, position: number, beforeGroupId?: string, category: TableSourceCategory = 'Preprocessed Constants') => {
  ...
  const moveLibraryItem = (
    ...
    if (kind === 'source') {
      ...
      onConfigChange({ ...config, dataSources: result.items.map((source) => source.id === config.dataSources[libraryItemDrag.itemIndex]?.id ? { ...source, category: category ?? config.dataSourceGroups.find((group) => group.id === groupId)?.category ?? config.dataSources[targetIndex]?.category ?? source.category } : source), dataSourceGroups: result.groups });
    } else if (kind === 'lookup') {
      const result = moveLibraryCollection(
        config.lookups,
        config.lookupGroups,
  ...
  const moveLibraryGroup = (
    ...
    if (kind === 'source') {
      ...
      onConfigChange({ ...config, dataSources: result.items, dataSourceGroups: result.groups.map((group) => group.id === libraryGroupDrag.groupId ? { ...group, category: category ?? (target.type === 'group' ? config.dataSourceGroups.find((entry) => entry.id === target.groupId)?.category : target.type === 'item' ? config.dataSources.find((entry) => entry.id === target.itemId)?.category : group.category) ?? 'Preprocessed Constants' } : group) });
    } else if (kind === 'lookup') {
  ...
  const editTableRelation = (relationId: string, dataSourceId: string, anchor: NonNullable<typeof relationEditor>['anchor']) => {
    ...
    setRelationEditor({
      ...
      principalDataSourceId: relation.principalDataSourceId,
  ...
  const saveTableRelation = () => {
    ...
    if (relationEditor.cardinality !== 'oneToMany' && ![sourceId, targetId].includes(relationEditor.principalDataSourceId ?? '')) return;
    ...
    const relation: TableRelation = {
      ...
      ...(relationEditor.cardinality !== 'oneToMany' ? { principalDataSourceId: relationEditor.principalDataSourceId } : {})
    };
    if (previousRelation && previousRelation.cardinality === relation.cardinality &&
      relationPrincipalId(previousRelation, config.dataSources) === relationPrincipalId(relation, config.dataSources)) {
      relation.principalFieldExpanded = previousRelation.principalFieldExpanded;
      relation.collapsedPrincipalField = previousRelation.collapsedPrincipalField;
    }
    const nextRelations = [...config.tableRelations.filter((entry) => entry.id !== relation.id), relation];
    for (const updated of reconcileRelationFields([...workingSources.values()], nextRelations)) {
      workingSources.set(updated.id, updated);
    }
  ...
  const addDataSource = (insertionIndex = config.dataSources.length, groupId?: string, shiftGroupsAtInsertion = true, category: TableSourceCategory = 'Preprocessed Constants') => {
    const source: DataSource = { category, id: createLocalId('source'), name: 'New data source', spatialUnit: '', fields: [], fieldGroups: [] };
  ...
  const renderLibraryInsertActions = (
    ...
    category: TableSourceCategory = config.dataSourceGroups.find((group) => group.id === (groupId ?? beforeGroupId))?.category ?? config.dataSources[position]?.category ?? 'Preprocessed Constants'
  ) => (
    <div
      ...
      onDrop={(event) => {
        ...
        if (libraryGroupDrag?.kind === kind) {
          if (beforeGroupId) {
            moveLibraryGroup(kind, { type: 'group', groupId: beforeGroupId, position: 'before' }, category);
          } else {
          ...
          moveLibraryItem(kind, position, 'before', groupId, shiftGroupsAtPosition, category);
        }
        clearLibraryDrag();
      }}
    >
      <button
        ...
        type="button"
        onClick={() => kind === 'source'
          ? addDataSource(position, groupId, shiftGroupsAtPosition, category)
          : kind === 'lookup'
      ...
      <button
        ...
        type="button"
        onClick={() => addLibraryGroup(kind, position, beforeGroupId, category)}
        ...
        >
          ...
          >
            ...
            {(() => {
              ...
              const renderRelationEditor = (className = '') => relationEditor ? <RelationPopover className={className} label={relationEditor.relationId ? 'Edit relationship' : 'Add relationship'}>
                ...
                {relationEditor.cardinality !== 'oneToMany' ? <>
                  <div className="field relation-role-control">
                    <span>This table: {source.name || 'Untitled table'}</span>
                    <div className="field-relation-cardinality relation-role-toggle" role="group" aria-label="This table's relationship role">
                      <button className={relationEditor.principalDataSourceId === source.id ? 'is-active' : ''} aria-pressed={relationEditor.principalDataSourceId === source.id} type="button" onClick={() => setRelationEditor((current) => current ? { ...current, principalDataSourceId: source.id } : current)}>Principal</button>
                      <button className={relationEditor.principalDataSourceId === relationEditor.targetDataSourceId ? 'is-active' : ''} aria-pressed={relationEditor.principalDataSourceId === relationEditor.targetDataSourceId} type="button" onClick={() => setRelationEditor((current) => current ? { ...current, principalDataSourceId: current.targetDataSourceId } : current)}>Secondary</button>
                    </div>
                  </div>
                  <small className="field-relation-note">{relationEditor.principalDataSourceId ? `Secondary table: ${config.dataSources.find((entry) => entry.id === (relationEditor.principalDataSourceId === source.id ? relationEditor.targetDataSourceId : source.id))?.name || 'Untitled table'}. ` : "Choose this table's role. "}The secondary table gets a virtual {relationEditor.cardinality === 'manyToMany' ? 'collection of principal IDs' : 'principal ID field'}. The principal field can be expanded below the table.</small>
              ...
              const renderFieldRow = (field: DataSourceField, fieldIndex: number, groupId?: string) => {
                ...
                >
                  ...
                  <div className="data-source-field-actions">
                    ...
                    {field.generatedRelationId ? <button className="mini-icon-button danger" type="button" title="Delete linked field and its relation" onClick={() => deleteField(sourceIndex, fieldIndex)}><Trash2 size={12} /></button> : <button
                      className="mini-icon-button danger"
                      type="button"
                  ...
                  {expanded ? (
                    <div className="data-source-expander-body" id={`data-source-body-${source.id}`}>
                      ...
                      >
                        ...
                        {sourceRelations.some((relation) => relationPrincipalId(relation, config.dataSources) === source.id) ? <div className="table-principal-relations" role="group" aria-label={`Joins for ${source.name || 'this table'}`}>
                          {sourceRelations.filter((relation) => relationPrincipalId(relation, config.dataSources) === source.id).map((relation) => {
                            const otherId = relation.sourceDataSourceId === source.id ? relation.targetDataSourceId : relation.sourceDataSourceId;
                            const otherName = config.dataSources.find((entry) => entry.id === otherId)?.name || 'Untitled table';
                            const cardinality = relation.cardinality === 'oneToOne' ? '1:1' : relation.cardinality === 'manyToMany' ? 'N:N' : '1:N';
                            const editorOpen = sourceRelationEditorOpen && relationEditor?.anchor === 'tableLink' && relationEditor.relationId === relation.id;
                            return <div className="table-principal-relation" key={relation.id}>
                              <button className="secondary-action tiny relation-expand-toggle" type="button" aria-label={`${relation.principalFieldExpanded ? 'Collapse' : 'Expand'} field for join with ${otherName}`} aria-expanded={Boolean(relation.principalFieldExpanded)} onClick={() => {
                                setRelationEditor(null);
                                onConfigChange(setPrincipalFieldExpanded(config, relation.id, !relation.principalFieldExpanded));
                              }}><ChevronDown size={12} className={relation.principalFieldExpanded ? 'is-expanded' : ''} />{relation.principalFieldExpanded ? 'Collapse field' : 'Expand field'}</button>
                              <button className="relation-field-badge" type="button" aria-label={`Edit relationship with ${otherName}`} aria-haspopup="dialog" aria-expanded={editorOpen} onClick={() => editTableRelation(relation.id, source.id, 'tableLink')}>Edit Link</button>
                              <span>to <strong>{otherName}</strong> · {cardinality} · {relation.cardinality === 'oneToMany' ? 'One side' : 'Principal'}</span>
                              {editorOpen ? renderRelationEditor('table-link-relation-popover') : null}
                            </div>;
                          })}
                        </div> : null}
                        <div className="field-final-actions">
              ...
              >
                ...
                {config.dataSources.flatMap((source, sourceIndex) => {
                  ...
                  const isUngrouped = !groupedDataSourceIds.has(source.id) && (source.category ?? 'Preprocessed Constants') === category;
                  return [
                    ...groupsAtPosition.flatMap((group) => [
                ...
                {config.dataSourceGroups.filter((group) => group.position === config.dataSources.length && (group.category ?? 'Preprocessed Constants') === category).flatMap((group) => [
                ...
                {config.dataSources.length > 0 || config.dataSourceGroups.length > 0
                  ? renderLibraryInsertActions('source', config.dataSources.length, `after-final-source-groups-${category}`, undefined, false, undefined, category)
                  : null}
                <div className="library-final-actions">
                  <button className="secondary-action tiny" type="button" onClick={() => addDataSource(config.dataSources.length, undefined, true, category)}><Plus size={11} /> Add source table</button>
                  <button className="secondary-action tiny" type="button" onClick={() => addLibraryGroup('source', config.dataSources.length, undefined, category)}><Plus size={11} /> Add group</button>
                </div>
              </div> : null}
              </section>);
            })()}
...
function KpiSourceEditor({
  ...
  const toggleKpi = (kpiId: string) => {
    ...
    if (existing) {
      onChange(kpi.sources.filter((item) => item.type !== 'kpi' || item.kpiId !== kpiId));
      return;
    }
  ...
  const pickerDataSourceGroups = config.dataSourceGroups
    ...
    .map((group) => ({
      ...
      category: group.category ?? 'Preprocessed Constants',
  ...
  const renderSelectedSourceRow = (item: KpiSourceItem, label: string) => {
    ...
    >
      ...
      <button className="mini-icon-button danger" type="button" title="Remove source" aria-label={`Remove source ${label}`} onClick={() => onChange(kpi.sources.filter((entry) => sourceSelectionKey(entry) !== sourceSelectionKey(item)))}><Trash2 size={12} /></button>
    </div>
    );
  };
